...
def make_env(args, idx, resume_run=False, generate_class=False, async_vec_env=False, params=None, eval_mode=False):
    def thunk():
        ...
        env_id = args.eval_env_id if eval_mode else args.env_id
        env_type = args.action_type.lower()
...
def make_env_for_gym(args, idx, resume_run=False, generate_class=False, async_vec_env=False, params=None, eval_mode=False):
    def thunk():
        ...
        if args.use_heuristic:
            if args.heuristic_type == "idle":
                gym_env = GridOpIdleForGym(gym_env)
            elif args.heuristic_type == "reco_revert":
                gym_env = GridOpRecoAndRevertBus(gym_env)
            elif args.heuristic_type == "idle_non_loop":
                gym_env = GridOpIdleNonLoop(gym_env)
...
def make_env_TOPOLOGY_Idle_LineSoftMaxRootMarginReward(args, idx, resume_run=False, generate_class=False, async_vec_env=False, params=None):
    def thunk():
        ...
        cr.addReward("LineSoftMaxRootMarginRewardUpgraded", LineSoftMaxRootMarginRewardUpgraded(
            use_softmax=False,
            temperature_softmax=1.0,
            n_th_root_safe=7,
            n_th_root_overflow=7
        ), 0.6)        
        '''
...
def make_env_TOPOLOGY_Idle_RL2GridLineMarginReward_BSLINE_TRULY(args, idx, resume_run=False, generate_class=False, async_vec_env=False, params=None):
    def thunk():
...
def make_env_TOPOLOGY_RecoAndRevert_LineSoftMaxRootMarginReward(args, idx, resume_run=False, generate_class=False, async_vec_env=False, params=None):
    def thunk():
...
def make_env_TOPOLOGY_RECO_REVERT_L2RPNRewardRegularized_BSLINE_TRULY(args, idx, resume_run=False, generate_class=False, async_vec_env=False, params=None):
    def thunk():
