...
class LinearSupport:
    ...
    def compute_corner_weights(self) -> List[np.ndarray]:
        ...
        def compute_poly_vertices(A, b):
            ...
            # Iterate over all combinations of constraints
            for combination in itertools.combinations(range(num_constraints), num_variables):
                A_comb = A[list(combination), :]
                b_comb = b[list(combination)]
                try:
                    vertex = np.linalg.solve(A_comb, b_comb)
                    if np.all(A @ vertex <= b + 1e-7):  # Check if the vertex is feasible
                        if not any(np.allclose(vertex, v, atol=1e-7) for v in vertices):
